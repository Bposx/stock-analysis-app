...
export default function ThaiStocks() {
  ...
  const { data: result, isLoading, refetch, isFetching } = useCategoryStocks("thai");
  const quotes: Quote[] = result?.data ?? [];

  // SET Index
  const setIndex = quotes.find((q) => q.symbol === "^SET.BK");
  const stockQuotes = quotes.filter((q) => q.symbol !== "^SET.BK");

  const sectors = useMemo(() => {
    return [
      { key: "all", label: t("table.allSectors") },
      ...getMarketSectorsList(THAI_STOCKS_DATA, language),
    ];
  }, [language, t]);
  ...
  const filtered = useMemo(() => {
    return stockQuotes
      .filter((q) => {
        const info = THAI_STOCKS_DATA[q.symbol];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-gray-100">{t("dashboard.thaiMarket")}</h1>
          </div>
          <p className="text-sm text-gray-500 mt-1">
            {t("market.setSubtitle")}
          </p>
        </div>
        <button
          onClick={() => refetch()}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="card p-4 border-l-4 border-l-brand-500">
          <p className="text-xs text-gray-500 mb-1">SET Index (^SET.BK)</p>
          <p className="font-mono text-2xl font-bold text-gray-100">
            {setIndex?.price?.toLocaleString("en", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) ?? "—"}
          </p>
          <div className="flex items-center gap-2 mt-1">
            <span className={clsx("text-xs font-semibold", (setIndex?.change_pct ?? 0) >= 0 ? "text-up" : "text-down")}>
              {(setIndex?.change_pct ?? 0) >= 0 ? "+" : ""}{setIndex?.change?.toFixed(2)} ({(setIndex?.change_pct ?? 0) >= 0 ? "+" : ""}{setIndex?.change_pct?.toFixed(2)}%)
            </span>
          </div>
        </div>
            ...
            >
              ...
              <div className="mt-3 flex items-baseline justify-between">
                <span className="font-mono text-lg font-bold text-gray-100">
                  ...
                  <span className="text-xs text-gray-500 ml-1 font-normal">THB</span>
                </span>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-border bg-surface-hover/50 text-xs text-gray-400">
                ...
                >
                  {t("table.price")} (THB) {sortField === "price" ? (sortDir === "asc" ? "↑" : "↓") : ""}
                </th>
                <th
                  className="px-4 py-3 text-right cursor-pointer hover:text-gray-200 select-none"
            ...
            <tbody className="divide-y divide-surface-border">
              {isLoading ? (
                ...
                filtered.map((stock) => {
                  ...
                  const info = THAI_STOCKS_DATA[stock.symbol];
                  const localizedName = getLocalizedCompanyName(stock.symbol, language);
                  const localizedSector = info ? getLocalizedSectorLabel(info.sectorKey, language) : "SET Equity";
                  return (
                    <tr
                      key={stock.symbol}
