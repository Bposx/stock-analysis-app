...
export default function ChinaStocks() {
  ...
  const { data: result, isLoading, refetch, isFetching } = useCategoryStocks("china");
  const quotes: Quote[] = result?.data ?? [];

  // SSE Composite Index
  const sseIndex = quotes.find((q) => q.symbol === "000001.SS");
  const stockQuotes = quotes.filter((q) => q.symbol !== "000001.SS");

  const sectors = useMemo(() => {
    return [
      { key: "all", label: t("table.allSectors") },
      ...getMarketSectorsList(CHINA_STOCKS_DATA, language),
    ];
  }, [language, t]);
  ...
  const filtered = useMemo(() => {
    return stockQuotes
      .filter((q) => {
        const info = CHINA_STOCKS_DATA[q.symbol];
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-gray-100">{t("dashboard.chinaMarket")}</h1>
          </div>
          <p className="text-sm text-gray-500 mt-1">
            {t("market.chinaSubtitle")}
          </p>
        </div>
        <button
          onClick={() => refetch()}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <div className="card p-4 border-l-4 border-l-brand-500">
          <p className="text-xs text-gray-500 mb-1">SSE Composite (000001.SS)</p>
          <p className="font-mono text-2xl font-bold text-gray-100">
            {sseIndex?.price?.toLocaleString("en", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) ?? "—"}
          </p>
          <div className="flex items-center gap-2 mt-1">
            <span className={clsx("text-xs font-semibold", (sseIndex?.change_pct ?? 0) >= 0 ? "text-up" : "text-down")}>
              {(sseIndex?.change_pct ?? 0) >= 0 ? "+" : ""}{sseIndex?.change?.toFixed(2)} ({(sseIndex?.change_pct ?? 0) >= 0 ? "+" : ""}{sseIndex?.change_pct?.toFixed(2)}%)
            </span>
          </div>
        </div>
            ...
            >
              ...
              <div className="mt-3 flex items-baseline justify-between">
                <span className="font-mono text-lg font-bold text-gray-100">
                  ...
                  <span className="text-xs text-gray-500 ml-1 font-normal">{q.currency || "CNY"}</span>
                </span>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-surface-border bg-surface-hover/50 text-xs text-gray-400">
                ...
                >
                  {t("table.price")} {sortField === "price" ? (sortDir === "asc" ? "↑" : "↓") : ""}
                </th>
                <th
                  className="px-4 py-3 text-right cursor-pointer hover:text-gray-200 select-none"
            ...
            <tbody className="divide-y divide-surface-border">
              {isLoading ? (
                ...
                filtered.map((stock) => {
                  ...
                  const info = CHINA_STOCKS_DATA[stock.symbol];
                  const localizedName = getLocalizedCompanyName(stock.symbol, language);
                  const localizedSector = info ? getLocalizedSectorLabel(info.sectorKey, language) : "China A-Shares";
                  const curr = stock.currency || "CNY";
                  return (
                    <tr
                      key={stock.symbol}
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 text-right font-mono font-bold text-gray-100">
                        {stock.price !== null && stock.price !== undefined
                          ? `${stock.price.toLocaleString("en", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} ${curr}`
                          : "—"}
                      </td>
                      ...
                      <td className="px-4 py-3.5 text-right font-mono text-xs text-gray-400 hidden sm:table-cell">
                        {stock.high ? stock.high.toFixed(2) : "—"} / {stock.low ? stock.low.toFixed(2) : "—"}
                      </td>
