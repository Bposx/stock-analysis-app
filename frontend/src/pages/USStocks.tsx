...
export default function USStocks() {
  ...
  const { data: result, isLoading, refetch, isFetching } = useCategoryStocks("us");
  ...
  const stockQuotes = quotes.filter((q) => !indexSymbols.includes(q.symbol));

  const sectors = useMemo(() => {
    return [
      { key: "all", label: t("table.allSectors") },
      ...getMarketSectorsList(US_STOCKS_DATA, language),
    ];
  }, [language, t]);
  ...
  const filtered = useMemo(() => {
    return stockQuotes
      .filter((q) => {
        const info = US_STOCKS_DATA[q.symbol];
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-gray-100">{t("dashboard.usMarket")}</h1>
          </div>
      ...
      {indices.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {indices.map((idx) => {
            ...
            return (
              <div key={idx.symbol} className="card p-4 border-l-4 border-l-brand-500">
                <p className="text-xs text-gray-500 mb-1">{names[idx.symbol] || idx.symbol}</p>
                <p className="font-mono text-2xl font-bold text-gray-100">
                  {idx.price?.toLocaleString("en", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </p>
                <div className="flex items-center gap-2 mt-1">
                  <span className={clsx("text-xs font-semibold flex items-center gap-0.5", isUp ? "text-up" : "text-down")}>
            ...
            <tbody className="divide-y divide-surface-border">
              {isLoading ? (
                ...
                filtered.map((stock) => {
                  ...
                  const info = US_STOCKS_DATA[stock.symbol];
                  const localizedName = getLocalizedCompanyName(stock.symbol, language);
                  const localizedSector = info ? getLocalizedSectorLabel(info.sectorKey, language) : "US Equity";
                  return (
                    <tr
                      key={stock.symbol}
